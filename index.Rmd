...
<h4 style="text-align:center;font-family:Topedia Sans TW Beta;">更新時間：`r format(Sys.time(), "%Y/%m/%d %H:%M")` </h4>


```{r echo=F,results ='hide',message =FALSE,warning=FALSE}
library(sf);library(tmap);library(leaflet);library(dplyr);library(RColorBrewer)
```

<h5 style="text-align:center;font-family:Topedia Sans TW Beta;">14天內新增病例統計數</h5>
<h5 style="text-align:center;font-family:Topedia Sans TW Beta;">`r format(Sys.Date()-15, "%Y/%m/%d")`~`r format(Sys.Date()-1, "%Y/%m/%d")` </h5>
```{r echo=F,warning=F}
...
leaflet(height="600", width="100%") %>%  
  ...
  addPolygons(data=TW,
              ...
              label =~cases, labelOptions = labelOptions(noHide = T, textOnly = TRUE, textsize = "8px"),
              highlightOptions = highlightOptions(color = "#03F", weight = 3,fillOpacity = 0.8,bringToFront = TRUE),
              group = "鄉鎮市區")%>%
  addPolygons(data=TWC,
              ...
              label =~cases, labelOptions = labelOptions(noHide = T, textOnly = TRUE, textsize = "12px"),
...
<h5 style="text-align:center;font-family:Topedia Sans TW Beta;">2021/05/01~`r format(Sys.Date()-1, "%Y/%m/%d")` </h5>
```{r echo=F,warning=F}
